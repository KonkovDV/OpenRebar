...
public class AeroBimReportExporterTests
{
    ...
    public async Task ExportAsync_ShouldWriteAeroBimContractJson()
    {
        ...
        var outputPath = Path.Combine(Path.GetTempPath(), $"a101-aerobim-{Guid.NewGuid():N}.json");
        ...
        try
        {
            ...
            document.RootElement.GetProperty("$schema").GetString().Should().Be("aerobim-a101-reinforcement-report/v1");
