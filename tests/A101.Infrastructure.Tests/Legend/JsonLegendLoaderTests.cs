...
namespace A101.Infrastructure.Tests.Legend;
...
public class JsonLegendLoaderTests
{
    ...
    public async Task LoadAsync_RoundTripJson_ShouldLoadLegend()
    {
        ...
        var tempFile = Path.Combine(Path.GetTempPath(), $"a101-legend-{Guid.NewGuid():N}.json");
    ...
    {
        var loader = new JsonLegendLoader();
        var tempFile = Path.Combine(Path.GetTempPath(), $"a101-legend-invalid-{Guid.NewGuid():N}.json");
