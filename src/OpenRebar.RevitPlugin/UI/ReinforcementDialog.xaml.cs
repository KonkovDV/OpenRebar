...
namespace OpenRebar.RevitPlugin.UI;

/// <summary>
/// WPF dialog for the OpenRebar reinforcement generation plugin.
...
public partial class ReinforcementDialog : Window
{
    ...
    private async void BtnExecute_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            MessageBox.Show("Выберите файл изолиний.", "OpenRebar", MessageBoxButton.OK, MessageBoxImage.Warning);
            return;
        }
        ...
        try
        {
            ...
            {
                MessageBox.Show("Некорректная толщина плиты.", "OpenRebar", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }

            if (!double.TryParse(TxtCover.Text, out double cover) || cover <= 0)
            {
                MessageBox.Show("Некорректный защитный слой.", "OpenRebar", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
        ...
        catch (OpenRebarDomainException ex)
        {
            ...
            MessageBox.Show($"{ex.ErrorCode}\n{ex.Message}", "OpenRebar — Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
        }
        catch (Exception ex)
        {
            AppendStatus($"ОШИБКА: {ex.Message}");
            MessageBox.Show(ex.Message, "OpenRebar — Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
        }
        finally
        {
